...
export default function OrderTracking({ userorders }) {
  ...
  return (
    <div className="p-4 sm:p-6 flex flex-col items-center bg-gray-50 min-h-screen">
      <Card className="w-full max-w-6xl shadow-xl rounded-xl bg-white">
          ...
          {userorders.length === 0 ? (
            ...
            <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {userorders.map((order) => (
                <div
                  key={order.id}
                  className="cursor-pointer p-4 border rounded-lg hover:bg-gray-100 transition-colors duration-200"
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-3 sm:mt-4">
                    ...
                    <Badge
                      className={cn(
                        "px-2 sm:px-3 py-1 text-white text-xs sm:text-sm rounded-full",
          ...
          <Card className="w-full max-w-2xl sm:max-w-3xl lg:max-w-4xl">
            <CardContent className="p-4 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4 text-sm sm:text-base">
                ...
                <p>
                  <strong>Order Date:</strong> {formatDate(selectedOrder.created_at)} at{" "}
                  {formatTime(selectedOrder.created_at)}
                </p>
                <p>
                  <strong>Updated At:</strong> {formatDate(selectedOrder.updated_at)} at{" "}
                  {formatTime(selectedOrder.updated_at)}
                </p>
                <p className="flex items-center">
                  <strong>Total Amount:</strong>{" "}
                  <IndianRupee size={18} className="mx-1" />
                  {selectedOrder.total_amount}
                </p>
                <p className="flex items-center">
                  <strong>Paid Payment:</strong>{" "}
                  <IndianRupee size={18} className="mx-1" />
                  {selectedOrder.paid_payment}
                </p>
                <p className="flex items-center">
                  <strong>Pending Payment:</strong>{" "}
                  <IndianRupee size={18} className="mx-1" />
                  {selectedOrder.pending_payment}
                </p>
                <p>
              ...
              <Button onClick={closeModal} className="mt-4 w-full sm:w-auto">
                Close
              </Button>
            </CardContent>
          </Card>
